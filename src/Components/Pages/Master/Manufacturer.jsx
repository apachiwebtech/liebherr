...
const Manufacturer = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = users.filter((user) =>
      user.Manufacturer && user.Manufacturer.toLowerCase().includes(value)
    );
  ...
  const validateForm = () => {
    ...
    if (!formData.Manufacturer.trim()) { // Check if the title is empty
      newErrors.Manufacturer = "Manufacturer Field is required."; // Set error message if title is empty
    }
      ...
      if (confirmSubmission) {
        if (isEdit) {
          ...
          await axiosInstance.post(`${Base_Url}/putmanufacturer`, { ...formData, updated_by: updatedBy },{
            ...
            .catch(error => {
              if (error.response && error.response.status === 409) {
                setDuplicateError('Duplicate entry, Manufacturer already exists!'); // Show duplicate error for update
              }
            });
        } else {
          ...
          await axiosInstance.post(`${Base_Url}/postmanufacturer`, { ...formData, created_by: createdBy },{
            ...
            .catch(error => {
              if (error.response && error.response.status === 409) {
                setDuplicateError('Duplicate entry, Manufacturer already exists!'); // Show duplicate error for insert
              }
            });
        }
      }
              ...
              <div className="row mp0">
                <div className="col-6">
                  <form onSubmit={handleSubmit} style={{ width: "50%" }} className="text-left">
                    <div className="mb-3">
                      <label htmlFor="ManufacturerInput" className="input-field"> Manufacturer<span className="text-danger">*</span></label>
                      <input
                        type="text"
