...
const Ratecard = () => {
    ...
    const handleSearch = (e) => {
        ...
        const filtered = users.filter((user) =>
            user.Ratecard && user.Ratecard.toLowerCase().includes(value)
        );
    ...
    const validateForm = () => {
        ...
        if (!formData.Ratecard.trim()) { // Check if the Ratecard is empty
        ...
        try {
            ...
            if (confirmSubmission) {
                if (isEdit) {
                    ...
                    await axios.put(`${Base_Url}/putratedata`, { ...formData, updated_by: updatedBy })
                        .then(response => {
                    ...
                    await axios.post(`${Base_Url}/postratedata`, { ...formData, created_by: createdBy })
                        .then(response => {
                           // window.location.reload();
                           setFormData({
                            Ratecard : ''
                        ...
                        <div className="row mp0">
                            <div className="col-6">
                                <form onSubmit={handleSubmit} style={{ width: "50%" }} className="text-left">
                                    <div className="mb-3">
                                        <label htmlFor="RatecardInput" className="input-field" >Add Rate Card Matrix</label>
                                        <input
                                            type="text"
                                        ...
                                        {errors.Ratecard && <small className="text-danger">{errors.Ratecard}</small>}
                                ...
                                <table className='table table-bordered table dt-responsive nowrap w-100 table-css' style={{ marginTop: '20px', tableLayout: 'fixed' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ padding: '12px 15px', textAlign: 'center' }}>Rate Card Matrix</th>
