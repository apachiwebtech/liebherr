...
const ComplaintCode = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.complaintcode.trim()) { // Check if the complaintcode is empty
        ...
        try {
            ...
            if (confirmSubmission) {
                if (isEdit) {
                    ...
                    await axios.put(`${Base_Url}/putcomdata`, { ...formData, updated_by: updatedBy })
                        .then(response => {
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Complaintcode already exists!'); // Show duplicate error for update
                            }
                        });
                } else {
                    // For insert, include 'created_by'
                    await axios.post(`${Base_Url}/postdatacom`, { ...formData, created_by: createdBy })
                        .then(response => {
                           // window.location.reload();
                           setFormData({
                            complaintcode : ''
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Complaintcode already exists!'); // Show duplicate error for insert
                            }
                        });
                }
            }
                        ...
                        <div className="row mp0">
                            <div className="col-6">
                                <form onSubmit={handleSubmit} style={{ width: "50%" }} className="text-left">
                                    <div className="mb-3">
                                        <label htmlFor="ComplaintcodeInput" className="input-field" >Add Complaint Code</label>
                                        <input
                                            type="text"
                                        ...
                                        {errors.complaintcode && <small className="text-danger">{errors.complaintcode}</small>}
                                ...
                                <table className='table table-bordered table dt-responsive nowrap w-100 table-css' style={{ marginTop: '20px', tableLayout: 'fixed' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ padding: '12px 15px', textAlign: 'center' }}>Complaint Code</th>
                                    ...
                                    <tbody>
                                        {currentUsers.map((item, index) => (
                                            <tr key={item.id}>
                                                ...
                                                <td style={{ padding: '10px' }}>{item.complaintcode}</td>
