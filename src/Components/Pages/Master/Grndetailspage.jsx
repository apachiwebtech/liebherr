...
const Grndetailspage = () => {
    ...
    let { grn_no } = useParams()

    try {
        // Replace custom characters to make the string Base64-compatible
        grn_no = grn_no.replace(/-/g, '+').replace(/_/g, '/');

        // Decrypt the value using CryptoJS
        const bytes = CryptoJS.AES.decrypt(grn_no, secretKey);
    ...
    async function getgrndetails() {
        try {
            const res = await axiosInstance.post(`${Base_Url}/getgrndetails`, { grn_no: grn_no }, {
                headers: {
                                        ...
                                        <div class="card-body" style={{ paddingTop: 0 }}>
                                            ...
                                            >
                                                <li class="py-1" style={{ flex: "1 1 33.33%", padding: "10px" }}>
                                                    ...
                                                    &nbsp;<span class="value"><b>{value.grn_no}</b></span>
                                                </li>
                                                <li class="py-1" style={{ flex: "1 1 33.33%", padding: "10px" }}>
                                                    <span class="lable">Invoice No:</span>
                                                    &nbsp;<span class="value"><b>{value.invoice_no}</b></span>
                                                </li>
                                                <li class="py-1" style={{ flex: "1 1 33.33%", padding: "10px" }}>
                                                    <span class="lable">Invoice Date:</span>
                                                    &nbsp;<span class="value"><b>{formatDate(value.invoice_date)}</b></span>
                                                </li>
                                                <li class="py-1" style={{ flex: "1 1 33.33%", padding: "10px" }}>
                                                    <span class="lable">Received From:</span>
                                                    &nbsp;<span class="value"><b>{value.csp_name}</b></span>
                                                </li>
