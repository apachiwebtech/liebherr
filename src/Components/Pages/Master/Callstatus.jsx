...
const Callstatus = () => {
    ...
    const handleSearch = (e) => {
        ...
        const filtered = users.filter((user) =>
            user.Callstatus && user.Callstatus.toLowerCase().includes(value)
        );
    ...
    const validateForm = () => {
        ...
        if (!formData.Callstatus.trim()) { // Check if the Callstatus is empty
        ...
        try {
            ...
            if (confirmSubmission) {
                if (isEdit) {
                    ...
                    await axios.put(`${Base_Url}/putcalldata`, { ...formData, updated_by: updatedBy })
                        .then(response => {
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Callstatus already exists!'); // Show duplicate error for update
                            }
                        });
                } else {
                    // For insert, include 'created_by'
                    await axios.post(`${Base_Url}/postcalldata`, { ...formData, created_by: createdBy })
                        .then(response => {
                           // window.location.reload();
                           setFormData({
                            Callstatus : ''
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Callstatus already exists!'); // Show duplicate error for insert
                            }
                        });
                }
            }
                        ...
                        <div className="row mp0">
                            <div className="col-6">
                                <form onSubmit={handleSubmit} style={{ width: "50%" }} className="text-left">
                                    <div className="mb-3">
                                        <label htmlFor="CallstatusInput" className="input-field" >Add Call Status Code</label>
                                        <input
                                            type="text"
                                        ...
                                        {errors.Callstatus && <small className="text-danger">{errors.Callstatus}</small>}
                                ...
                                <table className='table table-bordered table dt-responsive nowrap w-100 table-css' style={{ marginTop: '20px', tableLayout: 'fixed' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ padding: '12px 15px', textAlign: 'center' }}>Call Status Code</th>
