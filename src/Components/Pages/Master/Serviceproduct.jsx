...
const Serviceproduct = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.Serviceproduct.trim()) { // Check if the Serviceproduct is empty
        ...
        try {
            ...
            if (confirmSubmission) {
                if (isEdit) {
                    ...
                    await axios.put(`${Base_Url}/putprodata`, { ...formData, updated_by: updatedBy })
                        .then(response => {
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Serviceproduct already exists!'); // Show duplicate error for update
                            }
                        });
                } else {
                    // For insert, include 'created_by'
                    await axios.post(`${Base_Url}/postprodata`, { ...formData, created_by: createdBy })
                        .then(response => {
                           // window.location.reload();
                           setFormData({
                            Serviceproduct : ''
                        ...
                        .catch(error => {
                            if (error.response && error.response.status === 409) {
                                setDuplicateError('Duplicate entry, Serviceproduct already exists!'); // Show duplicate error for insert
                            }
                        });
                }
            }
                        ...
                        <div className="row mp0">
                            <div className="col-6">
                                <form onSubmit={handleSubmit} style={{ width: "50%" }} className="text-left">
                                    <div className="mb-3">
                                        <label htmlFor="ServiceproductInput" className="input-field" >Add Service Product</label>
                                        <input
                                            type="text"
                                            ...
                                            placeholder="Enter Service Product "
                                           
                                        />

                                        {errors.Serviceproduct && <small className="text-danger">{errors.Serviceproduct}</small>}
                                ...
                                <table className='table table-bordered table dt-responsive nowrap w-100 table-css' style={{ marginTop: '20px', tableLayout: 'fixed' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ padding: '12px 15px', textAlign: 'center' }}>Service Product</th>
                                    ...
                                    <tbody>
                                        {currentUsers.map((item, index) => (
                                            <tr key={item.id}>
                                                ...
                                                <td style={{ padding: '10px' }}>{item.Serviceproduct}</td>
