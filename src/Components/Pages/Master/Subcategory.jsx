    ...
    // Step 1: Add this state to track errors
     ...
     const fetchCategory = async () => {
       try {
         ...
         setCategory(response.data); // categories data ko set kar rahe hain
       } catch (error) {
         console.error('Error fetching categories:', error); // Agar error aata hai to console me print karenge
       }
     };
           ...
           try {
             ...
             if (confirmSubmission) {
               if (isEdit) {
                 ...
                 await axios.put(`${Base_Url}/putsubcategory`, { ...formData })
                   .then(response => {
                     window.location.reload();
                   })
                   .catch(error => {
                     if (error.response && error.response.status === 409) {
                       setDuplicateError('Duplicate entry, Subcategory already exists!'); // Show duplicate error for update
                     }
                   });
               } else {
                 // For insert, include duplicate check
                 await axios.post(`${Base_Url}/postsubcategory`, { ...formData })
                   .then(response => {
                     window.location.reload();
                   })
                   .catch(error => {
                     if (error.response && error.response.status === 409) {
                       setDuplicateError('Duplicate entry, subcategory already exists!'); // Show duplicate error for insert
                     }
                   });
               }
             }
       ...
       <div className="row">
         ...
         <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                 ...
                 {errors.category_id && <small className="text-danger">{errors.category_id}</small>} {/* Show error for catgeoryselection */}
               </div>
           ...
           <table className='table table-bordered' style={{ marginTop: '20px', tableLayout: 'fixed' }}>
             <thead>
               <tr>
                 ...
                 <th style={{ padding: '12px 15px', textAlign: 'center' }}>Category</th>
                 <th style={{ padding: '12px 15px', textAlign: 'center' }}>Subcategory</th>
             ...
             <tbody>
               {currentUsers.map((item, index) => (
                 <tr key={item.id}>
                   ...
                   <td style={{ padding: '10px' }}>{item.category_title}</td>
